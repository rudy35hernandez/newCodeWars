...
function solve(s){
  let lowAlph = 'abcdefghijklmnopqrstuvwxyz'  /// create a lowercase library
...
var greet = function(name) {
  let lowName = name.toLowerCase().slice(1)  //// made name in lowercase, sliced beginning at index 1
...
In John's car the GPS records every s seconds the distance travelled from an origin (distances are measured in 
  an arbitrary but consistent unit). For example, below is part of a record with s = 15:
...
/// I added 4 new empty arrays and ran a loop that will push 4 elements in order to each arr.
...
our car is old, it breaks easily. The shock absorbers are gone and you think it can handle about 15 more bumps 
