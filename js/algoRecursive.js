...
function joinArr(arr, joinElement) {
  function recursion(index, endGoal) {
    /// this is our recursion function inside our function, parameter is index, which start is 0, and endgoal is empty string.
    endGoal += arr[index]; /// everytime the recursion function is called, we're adding arr[index] to the empty string.
    if (index === arr.length - 1) {
      /// here we set our condtion. If index reaches the arrays length-1 we return the next line
